...
> Aplica tras mergear PR `feat/mcp-registry-publish-v2` (incluye `server.json` + `mcpName` en `package.json` + bump `0.1.1`).
...
# Esperado:
# { name: '@alexendros/proton-mail-mcp', version: '0.1.1', mcpName: 'io.github.alexendros/proton-mail-mcp' }
```
...
```bash
npm view @alexendros/proton-mail-mcp version mcpName
# Esperado:
# 0.1.1
...
# Esperado:
# match: true | name: io.github.alexendros/proton-mail-mcp | version: 0.1.1 | pkg.version: 0.1.1
```

Las versiones de `server.json` raíz y `server.json.packages[0].version` deben coincidir con la version recién publicada en npm (`0.1.1`).
...
# ✓ Server io.github.alexendros/proton-mail-mcp version 0.1.1
```

## §6 · Verificación post-publish

```bash
curl "https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.alexendros/proton-mail-mcp" | jq .
# Debes ver tu server en .servers[]
# Verifica .servers[0].packages[0].identifier == "@alexendros/proton-mail-mcp"
# Verifica .servers[0].version == "0.1.1"
```
...
Tras éxito, append a:
1. `~/.claude/cuadernos/proton-mail-mcp/bitacora.md` (entrada `## 2026-05-XX HH:MM · v0.1.1 publicada en MCP Registry`)
...
Crear release `v0.1.1` en GitHub:

```bash
git tag v0.1.1
git push origin v0.1.1
gh release create v0.1.1 --notes-from-file <(awk '/## \[0.1.1\]/,/## \[0.1.0\]/' CHANGELOG.md | head -n -1)
```
